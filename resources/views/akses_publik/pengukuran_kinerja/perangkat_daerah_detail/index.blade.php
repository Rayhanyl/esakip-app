    ...
    <div class="section sec-services">
        <div class="container">
            <div class="col-12 mb-4">
                <a href="{{ route('aspu.pengukuran.kinerja') }}" class="fw-bold">
                    Back to pengukuran kinerja
                </a>
            </div>
            <form
                action="{{ route('aspu.pengukuran.kinerja.detail', ['user' => $user->id, 'tahun' => 'null', 'triwulan' => 'null']) }}"
                method="get">
                @csrf
                <div class="row g-3">
                                    ...
                                    <tbody>
                                        @foreach ($data as $item)
                                            <tr style="font-size:12px;">
                                                ...
                                                <td>
                                                    {{ $item->sasaran_strategis_indikator->indikator_sasaran_strategis }}
                                                </td>
                                                <td class="text-center">
                                                    @if ($item->sasaran_sub_kegiatan == null)
                                                        -
                                                    @else
                                                        {{ $item->sasaran_sub_kegiatan->sasaran_sub_kegiatan }}
                                                    @endif
                                                </td>
                                                <td class="text-center">
                                                    @if ($item->sasaran_sub_kegiatan_indikator == null)
                                                        -
                                                    @else
                                                        {{ $item->sasaran_sub_kegiatan_indikator->indikator_sub_kegiatan }}
                                                    @endif
                                                </td>
                                                <td class="text-center">
                                                    @if ($item->triwulan == 'tahun')
                                                        {{ $item->tahunan_target }}
                                                    @else
                                                        {{ $item->sasaran_sub_kegiatan_target }}
                                                    @endif
                                                </td>
                                                <td class="text-center">
                                                    @if ($item->triwulan == 'tahun')
                                                        {{ $item->tahunan_realisasi }}
                                                    @else
                                                        {{ $item->realisasi }}
                                                    @endif
                                                </td>
                                                ...
                                                <td class="text-center">
                                                    @if ($item->sasaran_sub_kegiatan_indikator == null)
                                                        -
                                                    @else
                                                        {{ $item->sasaran_sub_kegiatan_indikator->sub_kegiatan ?? '-' }}
                                                    @endif
                                                </td>
                                                <td class="text-center">
                                                    @if ($item->triwulan == 'tahun')
