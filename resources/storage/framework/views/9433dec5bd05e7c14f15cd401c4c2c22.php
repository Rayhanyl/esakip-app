<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>E-SAKIP</title>
    <link rel="stylesheet" href="<?php echo e(asset('assets/css/main/app.css')); ?>">
    <link rel="stylesheet" href="<?php echo e(asset('assets/css/main/style.css')); ?>">
    <link rel="stylesheet" href="<?php echo e(asset('assets/css/pages/auth.css')); ?>">
    <link rel="shortcut icon" href="<?php echo e(asset('assets/images/logo/pemkab_favicon.png')); ?>" type="image/x-icon" />
    <link rel="shortcut icon" href="<?php echo e(asset('assets/images/logo/pemkab_favicon.png')); ?>" type="image/png" />
</head>

<body>
    <div class="vh-100 background-img-login" id="auth">
        <div class="row">
            <div class="col-12 col-lg-7 bg-dark">
                <div class="row py-5 px-5 d-flex algin-items-center justify-content-center">
                    <div class="col-12">
                        <div class="row p-3 g-3">
                            <div class="col-12 col-lg-6">
                                <img class="w-100 rounded-3 shadow-5"
                                    src="<?php echo e(asset('assets/images/logo/picture3.jpg')); ?>" alt="">
                            </div>
                            <div class="col-12 col-lg-6">
                                <img class="w-100 rounded-3" src="<?php echo e(asset('assets/images/logo/picture5.jpg')); ?>"
                                    alt="">
                            </div>
                            <div class="col-12 col-lg-6">
                                <img class="w-100 rounded-3 shadow-5"
                                    src="<?php echo e(asset('assets/images/logo/picture6.jpg')); ?>" alt="">
                            </div>
                            <div class="col-12 col-lg-6">
                                <img class="w-100 rounded-3" src="<?php echo e(asset('assets/images/logo/picture7.jpg')); ?>"
                                    alt="">
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-12 col-lg-5">
                <div class="row g-3 py-5">
                    <div class="col-12 text-center">
                        <img class="w-25" src="<?php echo e(asset('assets/images/logo/pemkab1.png')); ?>" alt="">
                    </div>
                    <div class="col-12 text-center">
                        <h3>e-SAKIP</h3>
                        <p class="fw-semibold">Sistem Akuntabilitas Kinerja Instansi <br> Pemerintah Kabupaten
                            Majalengka</p>
                    </div>
                    <div class="col-12">
                        <form class="row g-3 d-flex justify-content-center" action="<?php echo e(route('auth.login')); ?>"
                            method="POST">
                            <?php echo csrf_field(); ?>
                            <div class="col-10">
                                <label for="email">Username</label>
                                <input type="email" class="form-control" name="email" id="email" required>
                            </div>
                            <div class="col-10">
                                <label for="password">Password</label>
                                <input type="password" class="form-control" name="password" id="password" required>
                            </div>
                            <div class="col-10 form-check">
                                <a href="<?php echo e(route('aspu.index')); ?>" class="form-check-label fw-bold text-success"
                                    for="exampleCheck1" style="text-decoration:d-none;">
                                    <i class="bi bi-arrow-left-circle-fill"></i> Back to landing page</a>
                            </div>
                            <div class="col-12 text-center">
                                <button type="submit" class="btn btn-success rounded-5 w-50">Masuk</button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11.11.0/dist/sweetalert2.all.min.js"></script>
</body>

</html>
<?php /**PATH C:\laragon\www\esakip-app\resources\views/auth/login.blade.php ENDPATH**/ ?>